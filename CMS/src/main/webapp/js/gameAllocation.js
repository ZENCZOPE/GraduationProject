function queryGameList(currPage) {
	...
	realCommitLog("/CMS/game/gameAllocation.do", jsonStr);
}
...
function showData(data) {
	...
	content += "<table class='table'><tr><td>赛程ID</td><td>赛程名称</td><td>赛程等级</td><td>赛程地址</td>"
			+ "<td>赛程日期</td><td>结束日期</td><td>参加人数</td><td>晋级人数</td><td>赛程状态</td><td>分配赛程</td></tr>";
	// 循环展示数据
	$.each(games, function(i, item) {
		...
		content += "<tr><td>" + item.gameId + "</td>"
				...
				+ "<td><button style='width: 70px;' onclick='makeGame(" + item.gameId + ")'>分配赛程</button></td></tr>";
	});
	// 表尾
	content += "</table>";
