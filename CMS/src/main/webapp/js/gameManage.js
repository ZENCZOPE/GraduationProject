...
function showData(data) {
	...
	var content = "";
	// 表头
	content += "<table class='table'><tr><td>赛程ID</td><td>赛程名称</td><td>赛程等级</td><td>赛程地址</td>"
			+ "<td>赛程日期</td><td>结束日期</td><td>参加人数</td><td>晋级人数</td><td>赛程状态</td><td>修改/删除</td></tr>";
	// 循环展示数据
	$.each(games, function(i, item) {
		...
		content += "<tr><td>" + item.gameId + "</td>"
				...
				+ "<td><button onclick='gameChange(" + item.gameId + ")'>修改</button>"
				+ "<button onclick='gameDelete(" + item.gameId + ")'>删除</button></td></tr>";
	});
	// 表尾
	content += "</table>";
...
function gameChange(gameId) {
	var index = parent.layer.open({
		title : [ '赛程ID: ' + gameId, 'font-weight:bold;' ],
		type : 2,
		area : [ '900px', '550px' ],
		fixed : false, // 不固定
		maxmin : false,
		content : '/CMS/game/getGameChangePage.do?gameId=' + gameId
	});
}
	...
	layer.confirm(
		...
		function(){
			...
			$.ajax({
				...
				url: "/CMS/game/ajaxGameDelete.do",
