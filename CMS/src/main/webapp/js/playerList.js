function queryPlayerList(currPage) {
	...
	realCommitLog('/CMS/user/ajaxGetPlayerList.do', jsonStr);
}
...
function showData(data) {
	...
	content += '<button onclick="queryPlayerList(1);"';
	if (data.pagenum == 1) {
	...
	content += '<button onclick="queryPlayerList(' + (data.pagenum - 1) + ');"';
	if (data.pagenum == 1) {
		content += 'class="button"  disabled';
	} else if (data.pagenum > 1) {
		content += ' class="button";'
	...
	content += '<button onclick="queryPlayerList(' + data.totalpage + ');"';
	if (data.pagenum == data.totalpage) {
		content += 'class="button"  disabled';
	} else if (data.pagenum < data.totalpage) {
		content += ' class="button";'
